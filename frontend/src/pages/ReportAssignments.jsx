...
export default function ReportAssignments() {
    ...
    const exportToExcel = () => {
        ...
        Object.values(groupedAssignments).forEach(({ ticket, assignments: ticketAssignments }) => {
            const data = ticketAssignments
                .sort((a, b) => new Date(a.date) - new Date(b.date))
                .map(assignment => ({
                    Fecha: formatDate(assignment.date),
                    'Hora Inicio': formatTime(assignment.start_time),
                    'Hora Fin': formatTime(assignment.end_time),
            ...
            data.push({
                ...
                'Hora Inicio': '',
                'Hora Fin': 'TOTAL',
                Horas: ticketAssignments.reduce((sum, a) => sum + parseFloat(a.hours), 0).toFixed(2),
            });

            // Create worksheet with ticket name
            const ws = XLSX.utils.json_to_sheet(data);
            const sheetName = ticket.title.substring(0, 30); // Excel sheet names max 31 chars
            XLSX.utils.book_append_sheet(wb, ws, sheetName);
        });

        // Create summary sheet
        const summaryData = Object.values(groupedAssignments).map(({ ticket, totalHours }) => ({
            Ticket: ticket.title,
            Prioridad: getPriorityLabel(ticket.priority),
            'Total Horas': totalHours.toFixed(2),
            'Fecha Inicio': ticket.start_date ? formatDate(ticket.start_date) : '-',
            'Fecha Término': ticket.calculated_end_date ? formatDate(ticket.calculated_end_date) : '-',
        }));
        ...
        // Save file
        const fileName = `Reporte_Asignaciones_${userName}_${new Date().toISOString().split('T')[0]}.xlsx`;
